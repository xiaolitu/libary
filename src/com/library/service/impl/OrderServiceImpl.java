...
public class OrderServiceImpl implements OrderServiceI{
	...
	public boolean shoppingCartSaveOrder(String[] ids, int userId) throws SQLException {
		...
		List<Map<String, Object>> sum = dao.query("select sum(price) as sum from(SELECT t_shopping_cart.count*t_book.price as price FROM t_shopping_cart,t_book where t_shopping_cart.book_id=t_book.id and user_id=? and t_shopping_cart.id in (?)) t", userId, sBuffer.substring(0, sBuffer.length()-1));
